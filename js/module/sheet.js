...
export const Sheet = {
	...
	activateListeners: function (sheet, html) {
		...
		html.find('.obsidian-death-successes .obsidian-radio, '
			...
			.click(evt => Sheet.setAttributeLevel(sheet, 'system.attributes.death.success', evt));
		html.find('.obsidian-death-failures .obsidian-radio, '
			+ '.obsidian-death-failures .obsidian-feature-use')
			.click(evt => Sheet.setAttributeLevel(sheet, 'system.attributes.death.failure', evt));
	...
	enrichItemDescriptions: function (sheet) {
		return Promise.all(sheet.actor.items.map(async item => {
			if (getProperty(item, "system.description.value") && !item.obsidian.display) {
				item.obsidian.display = await TextEditor.enrichHTML(item.system.description.value, {
					documents: true,
					links: true,
					rollData: sheet.actor.getRollData(),
			...
			for (const effect of effects) {
				for (const component of effect.components) {
					if (component.type === 'description' && !component.display) {
						component.display = await TextEditor.enrichHTML(component.raw, {
							documents: true,
							links: true,
							rollData: sheet.actor.getRollData(),
							secrets: sheet.actor.isOwner,
	...
	getSenses: function (data) {
		...
		data.hasSenses =
			Object.keys(data.senses)
				.map(sense => data.system.attributes.senses[sense])
				.filter(n => n != null)
				.reduce((n, acc) => acc + n, 0) > 0;
	},

	limitedDragDrop: function (sheet) {
		sheet.form.ondragenter = () => sheet.form.classList.add('obsidian-dragover');
		sheet.form.ondragleave = () => sheet.form.classList.remove('obsidian-dragover');
	},
				...
				create: {
					...
					callback: dlg =>
						sheet.actor.createEmbeddedDocuments(
							'Item', [new FormDataExtended(dlg.find('form')[0]).object],
							{renderSheet: true})
				}
			},
			default: 'create'
	...
	onAttackToggle: function (sheet, evt) {
		...
		const attack = sheet.actor.obsidian.components.get(uuid);
		const effect = sheet.actor.obsidian.effects.get(attack.parentEffect);
		...
		sheet.actor.updateEmbeddedDocuments(
			'Item', [OBSIDIAN.updateArrays(item._source, update)]);
	},
	...
	onAttune: function (sheet, evt) {
		...
		const attuned = !!item.system.attunement;
		sheet.actor.updateEmbeddedDocuments('Item', [{_id: item.id, 'system.attunement': attuned ? 0 : 2}]);
	},
	...
	onDragItemStart: function (sheet, event) {
		...
		if (target.classList.contains('obsidian-char-inv-header-currency')) {
			...
			event.dataTransfer.setData('currency-drop', null);
		}
	...
	onDrop: function (sheet, event) {
		$('.obsidian-dragover').removeClass('obsidian-dragover');

		let data;
		try {
			data = JSON.parse(event.dataTransfer.getData('text/plain'));
			if (data.type === 'Actor') {
				return dnd5e.applications.actor.ActorSheet5e.prototype._onDropActor.call(
					sheet, event, data);
	...
	onEffectToggled: function (sheet, evt) {
		...
		if (uuid) {
			const effect = sheet.actor.obsidian.effects.get(uuid);
			const item = sheet.actor.items.get(effect.parentItem);
			...
			const rules = sheet.actor.flags.obsidian.rules;
	...
	onEquip: function (sheet, evt) {
		...
		if (item.obsidian.equippable) {
			sheet.actor.updateEmbeddedDocuments(
				'Item', [{_id: id, 'system.equipped': !item.system.equipped}]);
		} else {
	...
	onSlotClicked: function (sheet, evt) {
		...
		const spells = sheet.actor.system.spells[spellKey];
		const used = spells.max - spells.value;

		if (n > used && (spells.tmp || 0) > 0) {
			return sheet.actor.update({[`system.spells.${spellKey}.tmp`]: spells.tmp - 1});
		}

		const newValue = spells.value + (n > used ? -1 : 1);
		sheet.actor.update({[`system.spells.${spellKey}.value`]: newValue});
	},
	...
	setAttributeLevel: function (sheet, prop, evt) {
		const current = getProperty(sheet.actor, prop);
	...
	setCondition: function (sheet, evt) {
		...
		const current = sheet.actor.obsidian.conditions[id];
		const existing =
	...
	setGlobalRoll: function (sheet, roll) {
		const current = sheet.actor.flags.obsidian.sheet.roll;
		let result = 'reg';
		if (roll !== current) {
			result = roll;
		}
	...
	splitItem: async function (sheet, li) {
		...
		const doSplit = async qty => {
			...
			sheet.actor.updateEmbeddedDocuments('Item', [{
				_id: item.id,
				'system.quantity': item.system.quantity - qty
			}]);
		};

		if (item.system.quantity < 3) {
			doSplit(1);
		} else {
	...
	sumCurrency: function (dlg, available) {
		const transfer = {};
		const remaining = {...available};

		dlg.querySelectorAll('.obsidian-input-num-lg').forEach(el => {
			const denom = el.name;
			...
			const transferred = Math.min(value, available[denom]);
			transfer[denom] = transferred;
			remaining[denom] -= transferred;
		});
	...
	transferCurrency: async function (from, to, event, data) {
		...
		const existing = src?.getFlag('obsidian', 'currency') ?? from.system.currency;
		const doTransfer = dlg => {
			const {transfer, remaining} = Sheet.sumCurrency(dlg[0], existing);
			if (src) {
				src.setFlag('obsidian', 'currency', remaining);
			} else {
				from.update({'system.currency': remaining});
			}
	...
	updateContainerEquipped: function (sheet, evt) {
		...
		item.update({'system.equipped': equipped === false});
		return false;
	},
