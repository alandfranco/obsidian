...
function handleMsg (payload) {
	...
	if (payload.action === 'SET.WORLD') {
		...
		message?.delete();
	} else if (payload.action === 'SET-FLAG.MESSAGE') {
		const message = game.messages.get(payload.id);
		message?.setFlag('obsidian', payload.key, payload.value);
	} else {
		const actor = ObsidianActor.fromUUID(payload.uuid);
		if (actor) {
			const data = Array.isArray(payload.data) ? payload.data : [payload.data];
			actor[`${payload.action.toLowerCase()}EmbeddedDocuments`](
				payload.entity, data, payload.options);
		}
	}
}
