...
export const ObsidianItems = {
	...
	consumeQuantity: function (actor, component, n, updates) {
		...
		if (component.parentEffect) {
			const effect = actor.obsidian.effects.get(component.parentEffect);
			if (!effect) {
				return;
			}
		...
		const quantity = item.system.quantity - n;
		if (quantity >= 0) {
			updates.push({_id: item.id, 'system.quantity': quantity});
		}
	},

	deleteConsumed: function (actor, updates) {
		const toDelete = new Set();
		for (const update of updates) {
			const qty = getProperty(expandObject(update), 'system.quantity');
			if (Number.isNumeric(qty) && qty <= 0) {
				toDelete.add(update._id);
			}
		}

		if (toDelete.size) {
			actor.deleteEmbeddedDocuments('Item', Array.from(toDelete));
		}
	},
	...
	produceSpellSlot: function (actor, slot, unlimited) {
		...
		const prop = isPact ? 'system.spells.pact' : `system.spells.spell${slot}`;
		const spell = getProperty(actor, prop);
	...
	resolveEffect: function (actor, options) {
		...
		if (options.spell && options.spellLevel == null) {
			...
			const component =
				actor.obsidian.components.get(spell.getFlag('obsidian', 'parentComponent'));

			if (component?.source === 'individual'
				&& (component.method === 'item'
					|| (component.method === 'innate' && !component.withSlot)))
			{
				options.spellLevel = spell.system.level;
				if (component.upcast) {
			...
			&& item.flags.obsidian.tags.thrown)
		{
			...
			if (attack?.mode === 'ranged') {
				updates.push({_id: item.id, 'system.quantity': item.system.quantity - 1});
			}
		}
		...
		if (options.parentResolved === false) {
			...
			const roll = Rolls.create(actor, options);
			if (roll) {
				roll.then(() => ObsidianItems.deleteConsumed(actor, updates));
			} else {
				ObsidianItems.deleteConsumed(actor, updates);
			}
		}
		...
		if (!OBSIDIAN.notDefinedOrEmpty(item.getFlag('obsidian', 'ammo'))) {
			...
			if (ammo) {
				...
				ObsidianItems.roll(actor, {roll: 'item', id: ammo.id, suppressCard});
			}
		}
	...
	rollItem: async function (actor, options) {
		...
		if (item.type === 'feat' && item.system.activation.type === 'legendary') {
			const legact = actor.system.resources.legact;
			await actor.update({
				'system.resources.legact.value':
					Math.min(legact.max, legact.value + item.system.activation.cost)
			});
		}
	...
	rollSpellItem: function (actor, options) {
		...
		if (spell.getFlag('obsidian', 'parentComponent')) {
			component =
				actor.obsidian.components.get(spell.flags.obsidian.parentComponent);
		}
		...
		if (options.spellLevel == null) {
			if (spell.system.level > 0) {
				ObsidianItems.selectSpellLevel(actor, options, spell.system.level);
				return;
			} else {
	...
	rollToolItem: function (actor, options) {
		...
		const found = Object.values(actor.system.tools).find(t =>
