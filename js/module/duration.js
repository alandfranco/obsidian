	...
	if (effect.applies.length) {
		for (let target of targets) {
			...
			if (!target.actor.items.contents.some(item =>
				item.flags?.obsidian?.duration?.src === duration.id))
			{
...
export async function applyEffects (actor, effect, targets, on) {
	...
	if (duration) {
		active = duplicate(duration._source.flags.obsidian.active);
	}
...
async function createDuration (actor, rounds, effect, scaledAmount) {
	...
	if (!duration) {
		const item = actor.items.get(actor.obsidian.effects.get(effect)?.parentItem);
		const durationData = {
			icon: item?.img,
			label: 'Obsidian Duration',
			flags: {
				obsidian: {
					active: [],
					duration: true,
	...
	const newActive = duplicate(duration._source.flags.obsidian.active);
...
async function createActiveEffect (target, actor, effect, duration, on) {
	...
	effects.forEach(e => e.components = e.components.reduce((acc, component) => {
		if (component.type === 'item') {
			const id = randomID();
			const itemData = JSON.parse(component.itemData);
			itemData._id = id;
			setProperty(itemData, 'system.quantity', component.amount);
			appliedItems.push(itemData);
			component.itemData = id;
			acc.push(component);
		} else if (component.type !== 'condition' || component.temp) {
			acc.push(component);
	...
	const conditionImmunities = new Set(target.actor.obsidian.defenses.parts.conditions.imm);
	const conditions =
		permanentConditions
	...
	if (appliedItems.length) {
		for (const itemData of appliedItems) {
			setProperty(itemData, 'flags.obsidian.source', {type: 'item', item: item._id});
			await dispatchUpdate({
				target,
				action: 'CREATE',
				entity: 'Item',
...
function dispatchUpdate ({target, action, entity, data, options}) {
	data = Array.isArray(data) ? data : [data];
	target = target instanceof CONFIG.Token.documentClass ? target : target.document;

	if (game.user.isGM) {
		return target.actor[`${action.toLowerCase()}EmbeddedDocuments`](entity, data, options);
	} else {
		return game.socket.emit('module.obsidian', {
			action, entity, data, options,
			uuid: target.uuid
		});
	}
}
...
async function cleanupExpired (actor, expired) {
	const summonedTokens =
		canvas?.scene?.tokens.contents.filter(t => t.actorData.flags?.obsidian?.summon);

	for (const duration of expired) {
		const expiredTokens = summonedTokens.reduce((acc, t) => {
			const component =
				actor.obsidian.components.get(t.actorData.flags.obsidian.summon.parentComponent);
		...
		for (const uuid of duration.getFlag('obsidian', 'active') || []) {
			...
			const items =
				actor.items
					.filter(item => item.flags?.obsidian?.duration?.src === duration.id)
					.map(item => item.id);
...
async function onClick (evt) {
	...
	const ref = duration.flags.obsidian.ref;
	const effect = actor.obsidian.effects.get(ref);
	...
	if (effect) {
		...
		if (item?.type === 'spell') {
			ObsidianItems.rollItem(actor, {
				roll: 'item',
				id: item.id,
				spellLevel: item.system.level + (duration.flags.obsidian.scaledAmount || 0),
				parentResolved: true
			});
...
export function renderDurations () {
	...
	for (const duration of durations) {
		const effect = actor.obsidian.effects.get(duration.flags.obsidian.ref);
		if (!effect) {
			continue;
		}
