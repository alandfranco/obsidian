...
export const Categories = ['applied', 'rolls', 'resources', 'modifiers', 'special'];
...
export const Effect = {
	metadata: {
		...
		active: new Set([
			'roll-mod', 'bonus', 'defense', 'setter', 'multiplier', 'condition', 'item',
			'extra-damage'
		]),
	...
	determineRollMods: (actor, globalMod, pred) => {
		...
		const mods = actor.obsidian.filters.mods;
	...
	sheetGlobalRollMod: actor => Effect.makeModeRollMod(actor.flags.obsidian.sheet.roll),

	filterDamage: (actor, filter, dmg) => {
		let attackPred = filter => Filters.damage.isAttack(filter) && filter.multi === 'any';
		const parentEffect = actor.obsidian.effects.get(dmg.parentEffect);
	...
	fromMessage: msg => {
		...
		const effect = actor.obsidian.effects.get(msg.getFlag('obsidian', 'effectId'));
		if (!effect) {
			return [];
		}
	...
	isActive: function (item, effect) {
		...
		if (item.flags.obsidian.attunement && !item.system.attunement) {
			return false;
		}

		if (item.obsidian.equippable && !item.system.equipped) {
			return false;
		}
	...
	isConcentration: function (actor, effect) {
		...
		const item = actor.items.get(effect.parentItem);
		return item && item.type === 'spell' && item.system.components.concentration;
	},
	...
	scaleDamage: function (actor, scaling, scaledAmount, damage) {
		...
		if (typeof scalingEffect === 'string') {
			scalingEffect = actor.obsidian.effects.get(scalingEffect);
		}
