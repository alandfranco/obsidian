...
function createResourceSelect (config, bar) {
	...
	const itemsWithResources =
		config.actor.items.contents.filter(i =>
			i.flags.obsidian.effects.some(e => e.components.some(c => c.type === 'resource')));

	itemsWithResources.sort((a, b) => a.name.localeCompare(b.name));
	const entries =
		itemsWithResources.flatMap(i =>
			i.flags.obsidian.effects.flatMap(e =>
...
function getResourceData (actor, prop) {
	...
	const component = actor.obsidian?.components.get(uuid);
