...
export const Rolls = {
	...
	rollDamage: function (actor, damage, {item = null}) {
		...
		const diceParts = damage.map(dmg => {
			...
			const maxRoll = {
				mod: 0, ndice: ndice, die: dmg.die,
				roll: Array(ndice).fill(dmg.die),
				results: Array(ndice).fill({ result: dmg.die, success: true, discarded: false }) // Modify this based on your needs
			}];

			critParts.push(maxRoll);
			if (dmg.extraCrit?.ndice) {
