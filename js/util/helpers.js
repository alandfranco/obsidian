...
export function registerHandlebarHelpers () {
	...
	Handlebars.registerHelper('format-cover', function (cover) {
		return game.i18n.localize('OBSIDIAN.Cover').format(dnd5e.config.cover[cover]);
	});
	...
	Handlebars.registerHelper('format-recharge', function (actor, feature, options) {
		...
		if (feature.system.activation.type === 'legendary'
			&& feature.system.activation.cost
			&& feature.system.activation.cost > 1)
		{
			parts.push(
				feature.system.activation.cost
	...
	Handlebars.registerHelper('has-spells', function (actor, spellbook, level) {
		...
		const spell = actor.system.spells[`spell${level}`];
		let spellbookEntry;
		...
		return (spellbookEntry && spellbookEntry.spells.some(spell => spell.obsidian?.visible))
			|| (level > 0 && (Number(spell.max) || actor.system.spells.pact?.level === level));
	});
	...
	Handlebars.registerHelper('number-format', function (n) {
		return new Intl.NumberFormat(game.lang).format(n);
	});
	...
	Handlebars.registerHelper('sort', function (list, by, alreadySorted, options) {
		if (alreadySorted && options) {
			return list;
		}
	...
	Handlebars.registerHelper('which-damage', function (item, attack) {
		if (item.type === 'spell' && item.system.level < 1) {
	...
	Handlebars.registerPartial('select-class', `
		{{#if @root.actor}}
			...
			{{#select class}}
				{{#each @root.actor.obsidian.classes}}
