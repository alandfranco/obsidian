...
export class ObsidianNPC extends dnd5e.applications.actor.ActorSheet5eNPC {
	...
	activateListeners (html) {
		...
		const activateEditor =
			html.find('.editor > .editor-edit')[0].onclick;
	...
	async getData () {
		...
		data.isObject = data.system.details?.type?.value === 'object';
		data.featCategories = {};
		data.skills = {};
		data.summonLevel = this._getSummonLevel();
		data.actor.obsidian.tempEffects = [];

		for (const skill of Object.values(data.actor.system.skills)) {
		...
		for (const item of data.items) {
			let cat = item.system.activation?.type;
	...
	async activateEditor (name, options = {}, initialContent = '') {
		...
		return editor.instance = await TextEditor.create(options, initialContent || editor.initial).then(mce => {
			editor.mce = mce;
	...
	_formatCreatureType () {
		...
		const flags = this.actor.flags.obsidian;
		...
		if (system.details.type != null && typeof system.details.type === 'object') {
			let type = '';
			if (!OBSIDIAN.notDefinedOrEmpty(system.details.type.value)) {
				type = game.i18n.localize(`OBSIDIAN.CreatureType.${system.details.type.value}`);
			}

			if (!OBSIDIAN.notDefinedOrEmpty(system.details.type.swarm)) {
				type =
					game.i18n.localize('OBSIDIAN.CreatureSwarmPhrase')
						.format(
							game.i18n.localize(`OBSIDIAN.Size.${system.details.type.swarm}`),
							game.i18n.localize(
								`OBSIDIAN.CreatureTypePl.${system.details.type.value}`)
								.toLocaleLowerCase());
			}
		...
		if (system.details.type?.value !== 'object'
			&& (derived.details.tags || system.details.type?.subtype))
		{
			const tags =
				[derived.details.tags || '', system.details.type?.subtype || '']
		...
		if (!OBSIDIAN.notDefinedOrEmpty(flags.details.alignment1)
			...
			|| !OBSIDIAN.notDefinedOrEmpty(system.details.alignment))
		{
			comma = '<span>,</span> ';
			alignment =
		...
		return `
			<span>${game.i18n.localize(`OBSIDIAN.Size.${system.traits.size}`)}</span>
			${mainType}${subtype}${comma}${alignment}
		`;
	}

	_getSummonLevel () {
		const level = this.actor.flags.obsidian?.summon?.spellLevel;
	...
	_rollHD () {
		const hd = this.actor.flags.obsidian.attributes.hd;
		if (!hd.value || !hd.max) {
			return;
		}

		this.actor.rollHD([[1, this.actor.obsidian.attributes.hd.die]]);
	}
	...
	_useLegendaryAction (evt) {
		let used = this.actor.system.resources.legact.value;
		...
		this.actor.update({'system.resources.legact.value': used});
	}
}
