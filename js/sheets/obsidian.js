...
export class ObsidianCharacter extends dnd5e.applications.actor.ActorSheet5eCharacter {
	...
	activateListeners (html) {
		...
		html.find('.obsidian-inspiration')
			.click(this._toggleControl.bind(this, 'system.attributes.inspiration'));
		...
		html.find('.obsidian-edit-background').click(evt =>
			this.actor.items.get(
				evt.currentTarget.closest('.item')?.dataset.itemId)?.sheet.render(true));
	...
	async getData () {
		await Sheet.enrichItemDescriptions(this);
		const data = {
		...
		data.actor = this.actor.clone({}, {keepId: true});
		...
		for (const [id, skill] of Object.entries(data.system.skills)) {
			// Undo some upstream overwriting of skill labels
			skill.label = this.actor.system.skills[id].label;
		}

		data.actor.obsidian.feats =
			this.actor.obsidian.itemsByType.get('feat').map(i => i.toObject(false));

		data.background = this.actor.obsidian.itemsByType.get('background')?.[0];
		...
		data.speed.value = this.actor.system.attributes.movement[data.speed.label];
	...
	_cycleSpeed (evt) {
		...
		const currentSpeed = this.actor.flags.obsidian.attributes.speedDisplay || 'walk';
		const speeds = this.actor.system.attributes.movement;
	...
	_onAddFeature (evt) {
		...
		this.actor.createEmbeddedDocuments('Item', [{
			...
			system: {activation: {type: evt.currentTarget.dataset.action}},
			flags: flags
	...
	async _onSubmit (event, {preventClose = false} = {}) {
		if (event.currentTarget && event.currentTarget.dataset) {
			...
			if (name && name.startsWith('items.')) {
				...
				const item = this.actor._source.items[idx];
	...
	_reifyAttackLinks (atk) {
		atk.parentEffect = this.actor.obsidian.effects.get(atk.parentEffect);
		atk.parentEffect.damage =
			atk.parentEffect.components.filter(c => c.type === 'damage' && !c.versatile);
		atk.parentEffect.versatile =
			atk.parentEffect.components.filter(c => c.type === 'damage' && c.versatile);
		atk.parentItem = this.actor.items.get(atk.parentEffect.parentItem);
	}

	_rollHighestHD () {
		const highest = Config.HD.reduce((max, hd) => {
			const actorHD = this.actor.flags.obsidian.attributes.hd[`d${hd}`];
	...
	_setSaveProficiency (evt) {
		...
		let state = this.actor._source.system.abilities[save].proficient;
		...
		return this.actor.update({[`system.abilities.${save}.proficient`]: state});
	}
	...
	_setSkillProficiency (evt) {
		...
		const data = getProperty(this.actor.flags.obsidian, `${prop}.${id}`);
		...
		if (id.includes('.')) {
			...
			const newSkills = duplicate(this.actor._source.flags.obsidian[prop][key]);
	...
	_toggleControl (property, evt) {
		const state = !getProperty(this.actor, property);
	...
	async _updateObject (event, formData) {
		return super._updateObject(event, OBSIDIAN.updateArrays(this.actor._source, formData));
	}
}
