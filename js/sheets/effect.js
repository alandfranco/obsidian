...
export class ObsidianEffectSheet extends ObsidianItemSheet {
	...
	activateListeners (html) {
		...
		html.find('.obsidian-create-item-drop').on('dragover', evt => evt.preventDefault());
		html.find('.obsidian-create-item-drop')
			.each((i, el) => el.ondrop = this._onDropItem.bind(this));
		html.find('.obsidian-rm-create-item').click(this._onRemoveItem.bind(this));
		html.find('.obsidian-create-item-body')
			.click(evt => this._openEmbeddedEntity(evt, 'itemData', 'Item'));
	...
	async getData () {
		...
		data.config = {cover: dnd5e.config.cover};
		...
		if (data.item.flags?.obsidian?.effects?.length) {
			...
			components.filter(c => c.type === 'item').forEach(component => {
				if (component.itemData) {
					component.itemData = JSON.parse(component.itemData);
				}
			});
	...
	_editDescription (evt) {
		...
		const effects = duplicate(this.item.flags.obsidian.effects);
		const effect =
				...
				if (ed.mce) {
					...
					await this._updateEffects(
						OBSIDIAN.updateArrays(
							this.item._source, {'flags.obsidian.effects': effects}));
				}
			}
	...
	get _formData () {
		const expanded = OBSIDIAN.updateArrays(this.item._source, this._getSubmitData());
	...
	_openEmbeddedEntity (evt, collection, entity) {
		...
		const component =
			this.item._source.flags.obsidian.effects
				.flatMap(e => e.components)
		...
		const entities = component[collection];
		if (!entities) {
			return;
		}

		let data =
			Array.isArray(entities) ? entities.find(i => i._id === pill.dataset.id) : entities;
	...
	_onCheckBoxClicked () {
		...
		if (this.item.flags.obsidian?.type === 'ranged') {
			range.removeClass('obsidian-hidden');
			magic.addClass('obsidian-stacked-row');
		}
	}
	...
	async _onDropActor (evt) {
		...
		const effects = duplicate(this.item._source.flags.obsidian.effects);
		...
		if (data.type === 'Compendium') {
			...
			const entity = await Actor.implementation.fromDropData(data);
	...
	async _onDropItem (evt) {
		const [effects, component, item] = await this._processDrop(evt);
		if (['class', 'subclass', 'backpack', 'spell', 'background'].includes(item.type)) {
			return false;
		}

		component.itemData = JSON.stringify(item.toObject());
		this._updateEffects({'flags.obsidian.effects': effects});
	}
	...
	async _onDropSpell (evt) {
		...
		if (this.actor) {
			...
			component.spells.push(item.toObject());
		}
	...
	async _onDropTable (evt) {
		...
		component.tables.push(item.toObject());
		this._updateEffects({'flags.obsidian.effects': effects});
	}
	...
	async _onRemoveSelected (prop) {
		...
		if (orphanedSpells.length && this.actor) {
			await this.actor.deleteEmbeddedDocuments(
				'Item', orphanedSpells.filter(id => this.actor.items.get(id)));
		}
	...
	async _onRemoveItem (evt) {
		return this._removeEmbeddedEntity(evt, component => component.itemData = null);
	}
	...
	async _processDrop (evt, entity = 'Item') {
		...
		const item = await getDocumentClass(entity).fromDropData(data);
		if (!item) {
			return [];
		}
	...
	_setTrayState (newState) {
		...
		if (newState === TRAY_STATES.EFFECT) {
			...
			this.element.find('.obsidian-rm-effect').removeClass('obsidian-hidden');
			const component =
				this.item.flags.obsidian.effects
					.flatMap(e => e.components)
