...
export class ObsidianVehicle extends dnd5e.applications.actor.ActorSheet5eVehicle {
	...
	async getData () {
		...
		data.featCategories = {};
		data.availableCrew = data.actor.system.cargo.crew.map(crew => crew.name);
		data.availableCrew.sort();
		data.vehicleCapacity = this._formatVehicleCapacity();
		data.pacePerDay = this._formatPacePerDay();

		const motionlessPenalty = data.system.abilities.dex.mod;
		if (motionlessPenalty > 0) {
			data.motionlessAC = data.system.attributes.ac.value - motionlessPenalty;
		}
		...
		for (const item of data.items) {
			...
			if (item.type === 'feat') {
				cat = item.system.activation.type;
	...
	_formatPacePerDay () {
		const walk = this.object.system.attributes.movement.walk;
	...
	_formatVehicleCapacity () {
		const capacity = this.object.system.attributes.capacity;
		const flags = this.object.flags.obsidian;
		const crew = flags.crew
			? `${flags.crew} ${game.i18n.localize('DND5E.VehicleCrew').toLowerCase()}`
			: '';
	...
	async _onChangeInputDelta (event) {
		...
		let current = getProperty(item, prop);
	...
	_activateLayout (html) {
		...
		html.find('.obsidian-layout-item-qty').keyup(evt => {
			...
			if (type === 'items') {
				this.object.items.get(id).update({'system.quantity': qty});
			} else {
	...
	_adjustLayoutItemQty (evt, delta) {
		...
		if (type === 'items') {
			...
			const qty = item.system.quantity + delta;
			if (qty < 0) {
				item.delete();
			} else {
